...
این چالش بر طراحی یک **عملیات عددی جنریک و ایمن از نظر نوع (Type-Safe)** در زبان C# تمرکز دارد که بتواند با انواع مختلف عددی کار کند، بدون نیاز به بررسی نوع در زمان اجرا یا تبدیل نوع (Casting).

هدف این تمرین درک این موضوع است که **چرا و چه زمانی باید از جنریک‌ها همراه با محدودیت‌ها (Constraints) استفاده کرد** و این‌که C# مدرن چگونه امکان نوشتن منطقی یکسان و قابل اعتماد برای تمام انواع عددی را فراهم می‌کند.

---

## صورت مسئله

در برنامه‌های واقعی، اغلب نیاز داریم **یک قانون تجاری یکسان** را روی انواع عددی مختلف اعمال کنیم، مانند:

- `int`
- `decimal`
- `double`

روش‌های ساده و نادرست معمولاً شامل موارد زیر هستند:

- نوشتن چندین متد Overload شده
- بررسی نوع در زمان اجرا (Runtime)
- تبدیل نوع‌های ناایمن (Unsafe Casting)

همه‌ی این روش‌ها باعث کاهش **قابلیت نگهداری، خوانایی و صحت کد** می‌شوند.

---

## هدف

طراحی یک متد واحد که:

- **هر نوع عددی** را بپذیرد
- انواع غیرعددی را **در زمان کامپایل** رد کند
- یک قانون تجاری مشترک را اعمال کند (دو برابر کردن مقدار)
- نتیجه را **با همان نوع عددی ورودی** برگرداند
- از هرگونه اعتبارسنجی در زمان اجرا برای نوع‌ها
